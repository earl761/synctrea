<x-filament-panels::page>
    <div class="mb-6">
        ...
        <p class="text-gray-600">Here's an overview of your connections and tools.</p>
    </div>

    @php
        $user = auth()->user();
        if ($user && method_exists($user, 'isSuperAdmin') && $user->isSuperAdmin()) {
            $connectionPairs = \App\Models\ConnectionPair::with(['supplier', 'destination'])->get();
        } elseif ($user && $user->company_id) {
            $connectionPairs = \App\Models\ConnectionPair::with(['supplier', 'destination'])
                ->where('company_id', $user->company_id)
                ->get();
        } else {
            $connectionPairs = collect();
        }
    @endphp
